...
bool List::insert(const Patient& newElement){
	...
	while( index > 0 && elementPtr[firstDigit][index-1] > elementPtr[firstDigit][index] ){
...
//              If the removal is successful1, true is returned otherwise, false is returned.
...
bool List::remove( const Patient& toBeRemoved ){
	...
	while( index < elementCount[firstDigit] && duplicate == false ){ // To find index
		if( elementPtr[firstDigit][index] == toBeRemoved ){
			duplicate = true;
		}
		else{
			index++;
	...
	//Simple Swap so the deleted one is at an invalid elementcount 
	while( index < elementCount[firstDigit]-1 ){ 
		temp = elementPtr[firstDigit][index];
		elementPtr[firstDigit][index]   = elementPtr[firstDigit][index+1];
		elementPtr[firstDigit][index+1] = temp;
...
Patient* List::search(const Patient& target){
	...
	for( index = 0; index < elementCount[firstDigit]; index++ ){	//We cycle through the entire array based on element count
		compare = temp[index];										//Update compare with the next object

		if(compare == target){										//If we find a match 
